/*
1. 在 ET 边沿触发模式下，epoll函数在每次写端写入数据，也就是新数据到达时才返回，如果每次发送量大于接受量，接收端数据挤压,最终写端阻塞而死锁；
2. 在水平出发(默认模式)下，只要有未读完的数据，epoll函数就返回，接收端数据不会挤压；
3. 在 ET 边沿触发模式下，如果每次写入的少，读端读的多，则读端第二次调用read函数阻塞等待数据到达，当该客户端新的数据到达后read再次返回；
   如果该客户端迟迟不发送新的数据到达，由于没有机会执行epoll_wait函数，即使其他客户端写了新数据也没机会读到。
*/

// 如果有多个客户端，永远只有第一个客户端的数据能都被服务器读到，其它客户端的请求没有机会处理
...
int main()
{
    ...
    server_event.events = EPOLLIN | EPOLLET; /* ET 边沿触发，默认是水平触发 */
    ...
    while (1)
    {
        ...
        for (i = 0; i < ready_num; i++)
        {
            ...
            if (events[i].data.fd == server_fd && events[i].events == EPOLLIN)
            {
                ...
                //new_clinet_event.events = EPOLLIN;
            ...
            if (events[i].data.fd != server_fd && events[i].events == EPOLLIN)
            {
                ...
                {
                    printf("reading %s:%ld \n", __FILE__, __LINE__);
                    ret = read(events[i].data.fd, buffer, BUFFER_SIZE - 1);
                    printf("after read,ret = %d, %s:%ld \n", ret, __FILE__, __LINE__);
                    if (ret == -1)
                    {
                        perror("read");
                        break;
                    ...
                    ret = write(STDOUT_FILENO, buffer, strlen(buffer)); // 显示
                    if (ret == -1)
                    {
                        perror("write");
                        exit(-1);
                    ...
                    ret = write(events[i].data.fd, buffer, strlen(buffer));
                    if (ret == -1)
                    {
                        perror("write");
                        exit(-1);
